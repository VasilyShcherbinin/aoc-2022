...
fn main() -> io::Result<()> {
    ...
    for lines in reader_lines {
        ...
        if line_result.unwrap().contains("move") {
            ...
            let length_of_from_stack = init_stacks[from_stack].len();
            let popped_items: Vec<char> = init_stacks[from_stack].drain(length_of_from_stack - move_amount as usize..).collect();
            init_stacks[to_stack].extend(popped_items);
        }
    }
